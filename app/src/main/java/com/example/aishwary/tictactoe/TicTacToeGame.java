package com.example.aishwary.tictactoe;

import java.util.Random;

/**
 * Created by Aishwary on 9/16/2015.
 */
public class TicTacToeGame {
    ...
    public int getComputerMove() {
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            if (mBoard[i] != HUMAN_PLAYER && mBoard[i] != ANDROID_PLAYER) {
                ...
                char curr = mBoard[i];
                mBoard[i] = ANDROID_PLAYER;// check for the computer move and block this space for the computer
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            if (mBoard[i] != HUMAN_PLAYER && mBoard[i] != ANDROID_PLAYER) {
                char curr = mBoard[i]; // store the empty space in the local variable curr
        ...
        // if neither of these is true, then simply generate a random move
    ...
    public int checkForWinner() {
        //check for the horizontal lanes
        for (int i = 0; i <= 6; i += 3) {
